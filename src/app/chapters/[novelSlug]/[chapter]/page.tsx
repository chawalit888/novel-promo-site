    ...
    <article className="max-w-3xl mx-auto px-4 py-12">
      ...
      <div className="mt-12 bg-zinc-900 border border-zinc-800 rounded-xl p-6 text-center">
        ...
        <p className="text-zinc-400 text-sm mb-4">
          อ่านเรื่องเต็ม {novel.totalChapters} ตอนได้ที่ {novel.platform}
        </p>
        {novel.platformUrl ? (
          <a
            ...
            className="inline-block bg-amber-600 hover:bg-amber-500 text-zinc-950 font-semibold px-6 py-2 rounded-lg transition-colors"
          >
            อ่านต่อบน {novel.platform}
          </a>
        ) : (
          <p className="text-zinc-500 text-sm">เร็วๆ นี้บน {novel.platform}</p>
        )}
      </div>
    </article>
  );
